...
double GA::heritability = __DEFAULT_HERITABILITY__;
double GA::selection_strength = __DEFAULT_SELECTION_STRENGTH__;
...
// convert heritability into its inverse, plasticity
double GA::plasticity()
{
    double fraction_plastic = 1 - heritability;
    return __MAX_PLASTICITY__ * fraction_plastic;
    
    
    //return phenotypic_plasticity;
}
...
void GA::Organism::calculate_fitness()
{
    ...
    int randomFactor = random(0, _MAX_COLORFULNESS_);
    
    inverse_fitness = (sumDiff * 4) + invSumColor +
      (randomFactor * (GA::selection_strength - 1));
}
